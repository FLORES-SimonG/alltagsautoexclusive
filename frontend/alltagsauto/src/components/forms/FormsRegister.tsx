...
const validate = (values: IFormValues) => {
  ...
  if (!values.passwordVerify) {
    errors.passwordVerify = "Das Passwort ist erforderlich";
  } else if (values.passwordVerify !== values.password) {
    errors.passwordVerify = "Das Passwort stimmt nicht überein";
  }
  if (!values.name) {
    ...
    errors.name = "Der Name darf maximal 50 Zeichen lang sein";
  }
  ...
  if (!values.lastname) {
    ...
    errors.lastname = "Der Nachname darf maximal 50 Zeichen lang sein";
  }

  if (!values.phone) {
    errors.phone = "Die Telefonnummer ist erforderlich";
  } else if (isNaN(Number(values.phone))) {
    errors.phone = "Die Telefonnummer muss eine numerische Angabe sein";
  }
    ...
    >
      {({ errors, touched }) => (
        <Form className="text-sm text-textParagraph flex flex-col justify-center h-5/6">
          ...
          <ButtonPrimary
              title="Registrieren" size="lg" color="bg-secondary" 
            /> 
          </div>
        </Form>
      )}
